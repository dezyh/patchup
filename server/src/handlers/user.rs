...
pub async fn signup(user_signup: web::Json<UserSignup>, pool: web::Data<Pool>) -> Result<HttpResponse> {
    let user = user_signup.0;
    match services::user::signup(user, &pool) {
        Ok(token) => Ok(HttpResponse::Ok().json(
                ResponseBody::new(
...
pub async fn signout(req: HttpRequest, pool: web::Data<Pool>) -> Result<HttpResponse> {
    ...
    match auth {
        Some(auth) => {
            match services::user::signout(auth, &pool) {
        ...
        None => Ok(HttpResponse::BadRequest().json(ResponseBody::new(constants::MESSAGE_TOKEN_MISSING, constants::EMPTY)))
    }
}
